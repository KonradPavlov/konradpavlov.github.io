...
ui <- dashboardPage(
  ...
  dashboardBody(
    tags$head(tags$style(HTML("
      ...
      .nav-tabs { background-color: #e9ecef; border-radius: 5px; }
      .nav-tabs > li > a { color: #495057; }
      .nav-tabs > li.active > a { background-color: #ffffff; border-color: #ffffff; }
    "))),
    tabItems(
      # Onglet Données
              ...
              fluidRow(
                       ...
                       box(title = "⚙️ Configuration Avancée du Graphique",
                           ...
                           tabsetPanel(
                             ...
                             tabPanel("Mise en page",
                                      ...
                                      h5("📐 Marges (en pixels)"),
                                      fluidRow(
                                        column(3, numericInput("margin_top", "Haut :", value = 20, min = 0, max = 200, step = 5)),
                                        column(3, numericInput("margin_right", "Droite :", value = 0, min = 0, max = 100, step = 5)),
                                        column(3, numericInput("margin_bottom", "Bas :", value = 20, min = 0, max = 200, step = 5)),
                                        column(3, numericInput("margin_left", "Gauche :", value = 20, min = 0, max = 100, step = 5))
                                      )
                             ),
                             tabPanel("Options avancées",
                                      conditionalPanel(
                           ...
                           actionButton("preview_settings", "👁️ Prévisualiser les paramètres",
                                        class = "btn-info btn-block", style = "margin-bottom: 10px;"),
                           actionButton("reset_settings", "🔄 Réinitialiser",
                                        class = "btn-warning btn-block", style = "margin-bottom: 10px;"),
                ...
                column(8,
                       box(title = "📊 Graphique Généré", status = "success",
                           ...
                           plotOutput("current_plot_static", width = "100%"),
...
server <- function(input, output, session) {
  ...
  generate_enhanced_plot <- function(data, var, plot_type, group_var = NULL,
    ...
    if (!font_family %in% extrafont::fonts()) {
      showNotification(paste("Police", font_family, "non disponible. Utilisation de 'Arial' par défaut."), type = "warning")
      font_family <- "Arial"
    }
               ...
               if (detect_var_type(var_data_clean) == "continuous") {
                 ...
                 if (y_axis_type == "percent") {
                   ...
                   ggplot(data_plot, aes(x = var_cut, y = percent)) +
                     ...
                     geom_text(aes(label = sprintf("%.1f%%", percent)), vjust = -0.5, size = base_font_size / 3) +
                     labs(title = title, subtitle = subtitle, caption = caption,
                          x = if(xlab == "") var else xlab,
                   ...
                   data_plot <- data_plot %>%
                     count(var_cut)
                   ggplot(data_plot, aes(x = var_cut, y = n)) +
                     geom_col(fill = colors[1], alpha = 0.8) +
                     geom_text(aes(label = n), vjust = -0.5, size = base_font_size / 3) +
                     labs(title = title, subtitle = subtitle, caption = caption,
                          x = if(xlab == "") var else xlab,
                          y = if(ylab == "") "Effectifs" else ylab) +
                     base_theme
                 ...
                 if (!is.null(group_var) && group_var != "" && group_var %in% names(data)) {
                   if (y_axis_type == "percent") {
                     ...
                     ggplot(data_plot, aes(x = .data[[var]], y = percent, fill = .data[[group_var]])) +
                       ...
                       geom_text(aes(label = sprintf("%.1f%%", percent)), position = position_dodge(0.9), vjust = -0.5, size = base_font_size / 3) +
                       scale_fill_manual(values = colors) +
                       labs(title = title, subtitle = subtitle, caption = caption,
                            x = if(xlab == "") var else xlab,
                            y = if(ylab == "") "Pourcentage (%)" else ylab,
                     ...
                     data_plot <- data %>%
                       filter(!is.na(.data[[var]])) %>%
                       count(.data[[var]], .data[[group_var]])
                     ggplot(data_plot, aes(x = .data[[var]], y = n, fill = .data[[group_var]])) +
                       geom_col(position = "dodge", alpha = 0.8) +
                       geom_text(aes(label = n), position = position_dodge(0.9), vjust = -0.5, size = base_font_size / 3) +
                       scale_fill_manual(values = colors) +
                       labs(title = title, subtitle = subtitle, caption = caption,
                            x = if(xlab == "") var else xlab,
                            y = if(ylab == "") "Effectifs" else ylab,
                   ...
                   if (y_axis_type == "percent") {
                     ...
                     data_plot <- data %>%
                       filter(!is.na(.data[[var]])) %>%
                       count(.data[[var]])
                     ggplot(data_plot, aes(x = .data[[var]], y = n)) +
                       geom_col(fill = colors[1], alpha = 0.8) +
                       geom_text(aes(label = n), vjust = -0.5, size = base_font_size / 3) +
                       labs(title = title, subtitle = subtitle, caption = caption,
                            x = if(xlab == "") var else xlab,
             ...
             "bar_horizontal" = {
               ...
               if (y_axis_type == "percent") {
                 ...
                 ggplot(data_plot, aes(x = percent, y = reorder(.data[[var]], percent))) +
                   geom_col(fill = colors[1], alpha = 0.8) +
                   geom_text(aes(label = sprintf("%.1f%%", percent)), hjust = -0.1, size = base_font_size / 3) +
                   labs(title = title, subtitle = subtitle, caption = caption,
                 ...
                 ggplot(data_plot, aes(x = n, y = reorder(.data[[var]], n))) +
                   geom_col(fill = colors[1], alpha = 0.8) +
                   geom_text(aes(label = n), hjust = -0.1, size = base_font_size / 3) +
                   labs(title = title, subtitle = subtitle, caption = caption,
  ...
  current_plot <- reactive({
    req(current_data(), input$plot_var, input$plot_type)
    custom_colors <- NULL
  ...
  observeEvent(input$reset_settings, {
    ...
    updateNumericInput(session, "title_size", value = 16)
    updateSelectInput(session, "title_style", selected = "bold")
    updateNumericInput(session, "axis_title_size", value = 14)
    updateSelectInput(session, "axis_title_style", selected = "bold")
    updateNumericInput(session, "axis_text_size", value = 12)
    updateSelectInput(session, "axis_text_style", selected = "plain")
    updateNumericInput(session, "legend_text_size", value = 11)
    updateSelectInput(session, "legend_title_style", selected = "bold")
    updateSelectInput(session, "color_mode", selected = "palette")
    updateSelectInput(session, "color_palette", selected = "excel")
    updateColourInput(session, "single_color", value = "#5B9BD5")
    updateNumericInput(session, "n_colors", value = 5)
    updateSelectInput(session, "legend_position", selected = "right")
    updateRadioButtons(session, "y_axis_type", selected = "counts")
    updateNumericInput(session, "x_axis_rotation", value = 0)
    updateCheckboxInput(session, "show_grid_major", value = TRUE)
    updateCheckboxInput(session, "show_grid_minor", value = FALSE)
    updateSelectInput(session, "background_theme", selected = "white")
    updateNumericInput(session, "plot_width", value = 800)
    updateNumericInput(session, "plot_height", value = 600)
    updateNumericInput(session, "margin_top", value = 20)
    updateNumericInput(session, "margin_right", value = 20)
    updateNumericInput(session, "margin_bottom", value = 20)
    updateNumericInput(session, "margin_left", value = 20)
  ...
  output$download_word <- downloadHandler(
    filename = function() { paste0("rapport_", Sys.Date(), ".docx") },
    content = function(file) {
      plots <- plot_history()
      selected_items <- sapply(seq_along(plots), function(i) {
        if (is.null(input[[paste0("export_item_", i)]])) FALSE else input[[paste0("export_item_", i)]]
      ...
      doc <- officer::body_add_par(doc, paste("Généré le:", Sys.Date()), style = "Normal")
      for (i in seq_along(plots)) {
        if (selected_items[i]) {
          plot_info <- plots[[i]]
          temp_file <- tempfile(fileext = ".png")
          tryCatch({
            ggsave(temp_file, plot = plot_info$plot, device = "png", width = 12, height = 8, dpi = 300)
            doc <- officer::body_add_par(doc, paste("Graphique", i, ":", plot_info$title), style = "heading 2")
            doc <- officer::body_add_img(doc, src = temp_file, width = 6, height = 4)
            doc <- officer::body_add_par(doc, paste("Type:", plot_info$type), style = "Normal")
            doc <- officer::body_add_par(doc, paste("Variable:", plot_info$variable), style = "Normal")
            doc <- officer::body_add_par(doc, paste("Créé le:", format(plot_info$timestamp, "%d/%m/%Y %H:%M")), style = "Normal")
  ...
  observe({
    ...
    if (length(history) > 0) {
      ...
      updateSelectInput(session, "filter_plot_type", choices = c("Tous" = "all"))
      updateSelectInput(session, "filter_variable", choices = c("Toutes" = "all"))
    }
  })
  ...
  observe({
    ...
    for (i in seq_along(history)) {
      local({
        idx <- i
        observeEvent(input[[paste0("view_plot_", idx)]], {
          selected_preview(history[[idx]])
          showNotification(paste("Prévisualisation du graphique", idx), type = "message")
        })
        observeEvent(input[[paste0("delete_plot_", idx)]], {
          current_history <- plot_history()
          if (length(current_history) >= idx) {
            current_history <- current_history[-idx]
            plot_history(current_history)
            showNotification(paste("Graphique", idx, "supprimé"), type = "warning")
          }
        })
      })
    }
  ...
  observeEvent(input$preview_settings, {
    settings_summary <- paste0(
      "<h4>⚙️ Résumé des Paramètres Actuels</h4>",
      "<p><strong>Variable:</strong> ", input$plot_var, "</p>",
      "<p><strong>Type de graphique:</strong> ", input$plot_type, "</p>",
      "<p><strong>Police:</strong> ", input$font_family, " (Taille de base: ", input$base_font_size, ")</p>",
      "<p><strong>Mode de couleur:</strong> ", input$color_mode,
      if(input$color_mode == "palette") paste0(" (Palette: ", input$color_palette, ")") else "",
      if(input$color_mode == "single") paste0(" (Couleur: ", input$single_color, ")") else "",
      "</p>",
      "<p><strong>Légende:</strong> Position: ", input$legend_position, "</p>",
      "<p><strong>Marges:</strong> Haut=", input$margin_top, " | Droite=", input$margin_right,
      " | Bas=", input$margin_bottom, " | Gauche=", input$margin_left, "</p>"
    )
    showModal(modalDialog(
      title = "Prévisualisation des Paramètres",
      HTML(settings_summary),
      easyClose = TRUE,
